...
class Test__Log extends AccentTestCase {
    ...
    public function TestFileWriterWithResizing() {

        $SizeLimit= 10240;  // 10 kb
        $Message= str_repeat('1234567890', 20); // 200 bytes
        // fill log file to full capacity
        file_put_contents($this->LogFile, str_repeat('.', $SizeLimit));
        // store message 1000 times (200kb in total)
        // it should trigger resizing at least 100 times (1/4 of SizeLimit is 2,5kb - 10 messages will fill that)
        for($x=0; $x<1000; $x++) {
            // build logger each time to ensure triggering "ResizeFile()"
            $L= $this->Build(array('Writers'=>array('File'=>array(
                'Path'=> $this->LogFile,
                'SizeLimit'=> $SizeLimit,
            ))));
            // file size must be below $SizeLimit (plus size for one message and header)
            // and above 3/4 of $SizeLimit
            $Size= filesize($this->LogFile);
            $this->assertTrue($Size < $SizeLimit+strlen($Message)+100, 'Log size is greater then limit ('.$Size.' > '.$SizeLimit.')');
            $this->assertTrue($Size > $SizeLimit*0.75, 'Log size is smaller then 3/4  ('.$Size.' < '.($SizeLimit*0.75).')');
            // add message to file
            $L->Log($Message);
        }
    }
