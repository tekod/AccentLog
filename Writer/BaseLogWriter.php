...
abstract class BaseLogWriter extends Component {
    ...
    public function __construct($Options=array()) {
        ...
        $this->DateTime= [
            'TZ' => new \DateTime('now', new \DateTimeZone($this->GetOption('LoggerOptions.TimeZone'))),
    ...
    protected function GetHeader() {
        ...
        if ($this->Header === null) {
            ...
            $Header= "\n\n".$this->LoggerName
                    ."\n( timestamp: "
                    .$this->GetOption('LoggerOptions.TimeZone').'= "'.$this->FormattedDate('D, d M H:i:s', time()).'", '
                    .'UTC= "'.$this->FormattedDateUTC('D, d M H:i:s', time()).'" )'
    ...
    protected function FormattedDate($Format, $Timestamp) {
        ...
        $this->DateTime['TZ']->setTimestamp($Timestamp);
        return $this->DateTime['TZ']->format($Format);
    }


    /**
     * Return formatted date, in UTC timezone.
     *
     * @param string $Format
     * @param int $Timestamp
     * @return string
     */
    protected function FormattedDateUTC($Format, $Timestamp) {

        $this->DateTime['UTC']->setTimestamp($Timestamp);
        return $this->DateTime['UTC']->format($Format);
    }
